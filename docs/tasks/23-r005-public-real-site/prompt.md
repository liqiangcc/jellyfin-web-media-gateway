...
本文件只是 bootstrap/navigation，不是 Task Contract，也不保存动态 target site、Attempt 或 Evidence。
...
- Issue #14/R008 Final Acceptance 与 accepted security Evidence。

动态 GitHub 状态优先于聊天背景。

## Start Protocol

1. 确认 Issue 当前已经由 Coordinator 冻结 exact target site/public sample，并满足 `status:ready + env:cloud + no owner`。
...
5. 使用 accepted R008 `public_web` egress/Secret boundary；不得为了真实站点可用而增加私网例外或 open proxy。
6. re-resolve/expiry 必须消费 accepted R007 freshness 语义；不要定义第二套 item/media revision。
7. 保留 deterministic J1/J2 和 bounded real-site J3 Evidence；真实站点不可用时真实报告 BLOCKED/FAIL，不用 fixture 冒充 real-site PASS。
...
本 prompt 不得重新定义 Scope，也不得自行选择/更换 target site/sample。Candidate 变化后旧 real-site smoke 不自动证明新 Candidate。
