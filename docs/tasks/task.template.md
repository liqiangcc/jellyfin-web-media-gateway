...
Preferred verification backend: github-actions | cloud | wsl | windows | ubuntu-arm64 | manual-tv | none | capability-driven
Required capabilities: <capability list>
```

> GitHub Actions 是 execution / verification backend，不是会 claim Issue 的 Worker，因此不使用 `env:actions`。
>
...
Worker selection:
Web Worker first
→ Cloud / WSL / Windows only for missing worker capability
→ Ubuntu ARM64 / Real TV only for target-bound work

Verification backend selection:
GitHub Actions first
→ Cloud for long-running / repeated execution
→ WSL / Windows for interactive verification needs
→ Ubuntu ARM64 / Real TV only for target proof
```

说明为什么当前 Worker 与 Verification Backend 足够，或者缺少什么 capability 才需要下沉。
...
- 什么结果属于 FAIL；
- 什么缺失属于 BLOCKED；
- 当前 Worker 或 Verification Backend 缺 capability 时应路由到哪里；
- 恢复执行的最小条件。
...
2. 在 Issue 中记录 candidate SHA、实际 Orchestrator/Executor/Target、实际测试、未验证范围和 artifact/log；
