...
用户暂缓手机部署；当前功能部署主机为本机 x86_64 Linux VM（`grok-bot-vm-411252337`），iPhone 等客户端经 Tailscale tailnet 访问 Control/Display。具体 Task sequencing 以 `product-roadmap.md` 为准：本机部署验证访问链路与直链播放 → 恢复 #68 完成真实 B 站播放闭环。历史手机前置路由不阻塞此功能阶段，本文后续 Phase/完整设备退出条件仍保留。
