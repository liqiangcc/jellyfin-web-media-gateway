...
class AdmissionTests(unittest.TestCase):
    ...
    def test_admission_rejects_archive_before_execution(self):
        for attack in ('digest', 'candidate', 'missing', 'mutated', 'traversal', 'symlink', 'duplicate', 'quota', 'executable'):
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temp:
                ...
                with zipfile.ZipFile(archive, 'w') as z:
                    ...
                    if attack == 'quota':
                        z.writestr('cache/site-packages/extra', b'x')
                    if attack == 'executable':
                        info = zipfile.ZipInfo('cache/site-packages/extra')
                        info.create_system = 3
                ...
                with patch.object(a, 'ROOT', root), patch.object(a, 'context'), patch.object(a, 'owned_root_parent', return_value=Path(temp)):
                    with patch.object(a, 'MAX_FILES', 1 if attack == 'quota' else 6000), self.assertRaises(a.AdmissionError):
                        a.admit(archive, e)
    ...
    def test_valid_admission_and_recomputed_manifest_tamper(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / 'runtime'
            files = {k: {'size': 1, 'sha256': hashlib.sha256(b'x').hexdigest(), 'executable': k in a.EXECUTABLES} for k in a.REQUIRED}
            raw = json.dumps(dict(schema=1, candidate='a'*40, workflow_sha='a'*40, runtime=a.RUNTIME,
                source_root=str(root/'source'), target='x86_64-unknown-linux-gnu', run_id=1,
                run_attempt=1, files=files)).encode()
            archive = Path(temp) / 'artifact.zip'
            with zipfile.ZipFile(archive, 'w') as z:
                z.writestr('manifest.json', raw)
                for name in files:
                    z.writestr(name, b'x')
            e = dict(candidate='a'*40, run_id=1, run_attempt=1,
                manifest_sha256=hashlib.sha256(raw).hexdigest(), digest='sha256:'+a.file_hash(archive))
            with patch.object(a, 'ROOT', root), patch.object(a, 'context'), patch.object(a, 'owned_root_parent', return_value=Path(temp)):
                self.assertEqual(a.admit(archive, e)['result'], 'PASS')
                self.assertEqual((root/'bin/ytdlp-sandbox').stat().st_mode & 0o777, 0o700)
                # Attacker updates both local manifest and receipt; the control-plane
                # manifest digest still rejects before loading a helper/executable.
                (root/'manifest.json').write_text('{}')
                receipt = json.loads((root/'receipt.json').read_text())
                receipt['manifest_sha256'] = a.file_hash(root/'manifest.json')
