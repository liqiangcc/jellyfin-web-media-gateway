...
async function run() {
  ...
  evidence.production_path.push('Control refresh/event polling + Display reload/lease reconnect');
  evidence.claims.C7 = { same_session_after_refresh: true, stale_lease_status: staleLeaseStatus, new_page_lease_epoch: refreshedRegistration.page_lease_epoch };
  ...
  const staleCommand = await postJson(invalidForm, `/api/v1/sessions/${sessionId}/commands`, { request_id: 'e2e-stale-command', expected_session_revision: 0, command: { type: 'play' } });
  ...
  const replayFirst = await postJson(invalidForm, `/api/v1/sessions/${sessionId}/commands`, { request_id: 'e2e-replay-command', expected_session_revision: 4, command: { type: 'pause' } });
  const replaySecond = await postJson(invalidForm, `/api/v1/sessions/${sessionId}/commands`, { request_id: 'e2e-replay-command', expected_session_revision: 5, command: { type: 'stop' } });
  ...
  const resync = await invalidForm.evaluate(async id => (await fetch(`/api/v1/sessions/${encodeURIComponent(id)}/events?after=999999`)).json(), sessionId);
  ...
  evidence.claims.C8 = { invalid_source: true, offline_display_status: missingDisplay.status, stale_revision: staleCommand.payload.code, request_id_reuse: replaySecond.payload.code, missing_session_status: missingSession, event_resync: resync.snapshot_required };
