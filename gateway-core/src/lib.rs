...
<script>(()=>{
  ...
  const storageKey='gateway.tv.display.v1'; let registration=null, heartbeatTimer=null, renderingTimer=null, reconnecting=false, mediaError=null, renderingInFlight=false, renderingRequest=0, telemetrySequence=0, activationRequired=true;
  ...
  const tryPlay=async(context,trigger='remote')=>{if(!player.getAttribute('src')){show('No Gateway media is attached yet; waiting for a session.','warn');return false}overlay.classList.remove('playing');try{await player.play();activationRequired=false;show(trigger==='activation'?'Playback started.':'Playback started by Control.','ok');await sendCallback(context,'playing');overlay.classList.add('playing');return true}catch(error){mediaError=safeError(error);const activationBlocked=mediaError.name==='NotAllowedError';activationRequired=activationBlocked;show(activationBlocked?'Playback requires TV activation; use Press OK to play.':`Playback could not start (${mediaError.name}); use Press OK to retry.`,'warn');await sendCallback(context,'paused','playback_not_confirmed');return false}};
  const applyContext=async(context,previous,request,sourceChanged=false)=>{if(request!==renderingRequest||!currentContext(context))return;const changed=sourceChanged||!previous||context.session_revision!==previous.session_revision||contextKey(context)!==contextKey(previous);if(!changed)return;const positionChanged=previous&&Math.abs(context.position_ms-previous.position_ms)>250;if(positionChanged&&context.state!=='stopped'&&Number.isFinite(player.duration)&&context.position_ms<=player.duration*1000){player.currentTime=context.position_ms/1000}if(request!==renderingRequest||!currentContext(context))return;if(context.state==='playing'){if(activationRequired){show('Playback ready; use Press OK to activate the TV Display.','warn');return}await tryPlay(context)}else if(context.state==='paused'){activationRequired=false;player.pause();show('Playback paused by Control.','ok');await sendCallback(context,'paused')}else if(context.state==='stopped'){activationRequired=false;player.pause();if(Number.isFinite(player.duration)&&player.currentTime>0.05)player.currentTime=0;show('Playback stopped by Control.','ok');await sendCallback(context,'stopped')}};
