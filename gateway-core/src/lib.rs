...
impl GatewayService {
        ...
        Self {
            state: Arc::new(GatewayState {
                ...
                public_browser_worker:
                    browser_chromium::ChromiumBrowserWorker::for_public_observation(),
    ...
    #[cfg(test)]
    pub(crate) async fn create_public_session_with_worker<W: browser::BrowserWorker + 'static>(
        &self,
        request: CreateSessionRequest,
        worker: &W,
    ) -> source_session::CreationOutcome {
...
<script>(()=>{
  ...
  const storageKey='gateway.tv.display.v1'; let registration=null, heartbeatTimer=null, renderingTimer=null, reconnecting=false, mediaError=null, renderingInFlight=false, renderingRequest=0, telemetrySequence=0, activationRequired=true, lastAppliedPositionMs=null, pendingPosition=null;
  ...
  const contextKey=c=>c?`${c.session_id}:${c.item_id}:${c.item_revision}:${c.session_revision}:${c.display_generation}`:'';
  const currentContext=c=>Boolean(registration?.context&&contextKey(registration.context)===contextKey(c));
  const sendCallback=async(context,observation,errorCode=null)=>{if(!registration||!currentContext(context))return false;const position_ms=Number.isFinite(player.currentTime)?Math.max(0,Math.round(player.currentTime*1000)):context.position_ms;const sequence=Math.max(telemetrySequence,Number(context.telemetry_sequence)||0)+1;telemetrySequence=sequence;const body={lease_token:registration.lease_token,session_id:context.session_id,item_id:context.item_id,item_revision:context.item_revision,session_revision:context.session_revision,display_id:registration.display_id,display_generation:context.display_generation,telemetry_sequence:sequence,position_ms,observation,error_code:errorCode};try{const response=await fetch(`/api/v1/displays/${encodeURIComponent(registration.display_id)}/callback`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});return response.ok}catch(_){return false}};
  ...
  const tryPlay=async(context,trigger='remote')=>{if(!player.getAttribute('src')){show('No Gateway media is attached yet; waiting for a session.','warn');return false}overlay.classList.remove('playing');try{await player.play();activationRequired=false;show(trigger==='activation'?'Playback started.':'Playback started by Control.','ok');await sendCallback(context,'playing');overlay.classList.add('playing');return true}catch(error){mediaError=safeError(error);activationRequired=true;show(mediaError.name==='NotAllowedError'?'Playback requires TV activation (NotAllowedError); use Press OK to play.':`Playback could not start (${mediaError.name}); use Press OK to retry.`,'warn');await sendCallback(context,'paused','playback_not_confirmed');return false}};
  const applyPendingPosition=()=>{if(!pendingPosition||!currentContext(pendingPosition.context)||!Number.isFinite(player.duration)){return}const target=pendingPosition.position_ms;if(target>player.duration*1000){pendingPosition=null;return}player.currentTime=target/1000;lastAppliedPositionMs=target;pendingPosition=null};
  const applyContext=async(context,previous,request,sourceChanged=false)=>{if(request!==renderingRequest||!currentContext(context))return;const commandChanged=Boolean(previous&&context.session_revision!==previous.session_revision);const changed=sourceChanged||!previous||commandChanged||contextKey(context)!==contextKey(previous);if(!changed)return;const positionRequested=sourceChanged||commandChanged;if(context.state==='stopped'){pendingPosition=null}else if(positionRequested&&context.position_ms>=0){if(Number.isFinite(player.duration)&&context.position_ms<=player.duration*1000){player.currentTime=context.position_ms/1000;lastAppliedPositionMs=context.position_ms;pendingPosition=null}else if(!Number.isFinite(player.duration)){pendingPosition={context,position_ms:context.position_ms}}}if(request!==renderingRequest||!currentContext(context))return;if(context.state==='playing'){if(activationRequired){show('Playback ready; use Press OK to activate the TV Display.','warn');return}await tryPlay(context)}else if(context.state==='paused'){player.pause();show('Playback paused by Control.','ok');await sendCallback(context,'paused')}else if(context.state==='stopped'){player.pause();if(Number.isFinite(player.duration)&&player.currentTime>0.05)player.currentTime=0;show('Playback stopped by Control.','ok');await sendCallback(context,'stopped')}};
  const fetchRendering=async()=>{if(!registration||renderingInFlight)return;renderingInFlight=true;const request=++renderingRequest;try{const response=await fetch(`/api/v1/displays/${encodeURIComponent(registration.display_id)}/rendering`,{headers:{'x-display-lease':registration.lease_token},cache:'no-store'});if(request!==renderingRequest)return;if(response.status===404){registration.context=null;renderContext();return}if(!response.ok)throw new Error(`rendering ${response.status}`);const rendering=await response.json();const previous=registration.context;registration.context=rendering.context;telemetrySequence=Math.max(telemetrySequence,Number(registration.context.telemetry_sequence)||0);if(request!==renderingRequest)return;const stream=rendering.media?.streams?.find(item=>item.protocol==='http_file')||rendering.media?.streams?.[0];const sourceChanged=Boolean(stream?.gateway_path&&player.src!==new URL(stream.gateway_path,location.href).href);if(sourceChanged){player.src=stream.gateway_path;player.load()}capabilities.textContent=`Gateway media ready: ${rendering.media.title||'current item'}`;renderContext();await applyContext(registration.context,previous,request,sourceChanged)}catch(error){if(request===renderingRequest)show(`Display rendering reconnecting (${safeError(error).name}).`,'warn')}finally{renderingInFlight=false}};
  const heartbeat=()=>{if(heartbeatTimer)clearInterval(heartbeatTimer);if(!registration)return;const delay=Math.max(1000,Math.floor(registration.lease_ttl_ms/3));heartbeatTimer=setInterval(async()=>{try{const response=await fetch(`/api/v1/displays/${encodeURIComponent(registration.display_id)}/heartbeat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({lease_token:registration.lease_token})});if(!response.ok)throw new Error(`heartbeat ${response.status}`);await fetchRendering();if(!activationRequired)show(registration.context?'Display connected; session context retained.':'Display connected; waiting for a session.','ok')}catch(error){show(`Display heartbeat retrying (${safeError(error).name}).`,'warn')}},delay)};
  const register=async()=>{const old=readSaved();const body={display_id:displayId(),label:'TV Web Display',capabilities:['video','audio','subtitles'],previous_registration_id:old?.registration_id||null,previous_lease_token:old?.lease_token||null};try{const response=await fetch('/api/v1/displays/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});if(!response.ok){if(old){sessionStorage.removeItem(storageKey);return register()}throw new Error(`register ${response.status}`)}registration=await response.json();telemetrySequence=Number(registration.context?.telemetry_sequence)||0;save();capabilities.textContent='Gateway registration active; subtitles: '+(registration.context?'ready':'advertised');renderContext();await fetchRendering();heartbeat();if(renderingTimer)clearInterval(renderingTimer);renderingTimer=setInterval(fetchRendering,1000);return true}catch(error){registration=null;show(`Display reconnect waiting (${safeError(error).name}).`,'warn');return false}};
  document.querySelector('#activate').addEventListener('click',async()=>{const context=registration?.context;if(await tryPlay(context,'activation'))activationRequired=false});
  ...
  player.addEventListener('loadedmetadata',applyPendingPosition);
  player.addEventListener('error',()=>{show('Gateway media reported an error; waiting for recovery.','warn');const context=registration?.context;if(context)sendCallback(context,'unknown','media_incompatible')});
  ...
  window.__displayPrep={getRegistration:()=>registration,getStatus:()=>status.textContent,play:()=>tryPlay(registration?.context,'activation'),reconnect:register,getRendering:()=>registration?.context||null,getMediaState:()=>({readyState:player.readyState,duration:player.duration,currentTime:player.currentTime,paused:player.paused,error:player.error?.code||null,activationRequired,lastAppliedPositionMs})};
