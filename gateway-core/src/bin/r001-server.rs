...
const DEFAULT_MP4: &str = "https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4";
const DEFAULT_HLS: &str = "https://devstreaming-cdn.apple.com/videos/streaming/examples/img_bipbop_adv_example_ts/master.m3u8";

#[tokio::main]
async fn main() {
    let port: u16 = env::var("R001_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8787);
    let listener = TcpListener::bind(("127.0.0.1", port)).await.expect("bind R001 server");
    let addr = listener.local_addr().expect("listener addr");

    let mut registry = SiteAdapterRegistry::default();
    registry.register(Arc::new(GenericDirectAdapter)).expect("register generic-direct");
    ...
    let mp4_locator = registry.recognize(&mp4_input).expect("recognize public MP4");
    let mp4_media = registry.resolve(&mp4_locator).expect("resolve public MP4");
    let mp4_resource = GatewayService::resource_from_resolved(&mp4_media.streams[0], EgressScope::PublicWeb)
        .expect("public MP4 resource");
    let mp4_path = service.issue_path(Binding::new("r001-session", "public-mp4", 1, "primary"), mp4_resource, ttl);

    let hls_input = env::var("R001_PUBLIC_HLS").unwrap_or_else(|_| DEFAULT_HLS.into());
    let hls_locator = registry.recognize(&hls_input).expect("recognize public HLS");
    let hls_media = registry.resolve(&hls_locator).expect("resolve public HLS");
    let hls_resource = GatewayService::resource_from_resolved(&hls_media.streams[0], EgressScope::PublicWeb)
        .expect("public HLS resource");
    let hls_path = service.issue_path(Binding::new("r001-session", "public-hls", 1, "master"), hls_resource, ttl);
    ...
    let secret_path = fixture_path.map(|_| {
        ...
        secret_headers.insert("authorization", HeaderValue::from_static("Bearer r001-fixture-secret"));
        service.issue_path(
    ...
    println!("R001 source chain uses plugin={} site={}", mp4_locator.plugin_id, mp4_locator.site_id);
    axum::serve(listener, service.router()).await.expect("serve R001");
}
